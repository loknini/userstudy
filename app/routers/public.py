...
async def study_index(
    ...
    if study.status != "active":
        raise HTTPException(status_code=403, detail="问卷未激活或已暂停")
    
    # 获取配置
    config = service.get_study_config(study)
    if not config:
        raise HTTPException(status_code=500, detail="问卷配置加载失败")
...
async def start_study_by_code(
    ...
    return RedirectResponse(
        url=f"/study/{code}/question/0?pid={participant.id}",
        status_code=303
    )
...
async def show_question_legacy(
    ...
    return RedirectResponse(
        url=f"/study/default/question/{qidx}?pid={pid}",
        status_code=301
    )
...
async def submit_answer_legacy(
    ...
    # POST重定向需要307保持方法
    return RedirectResponse(
        url=f"/study/default/submit/{qidx}",
        status_code=307
    )
