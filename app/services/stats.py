...
from app.schemas import (
    StudyConfigData, OverallStats, QuestionStats, 
    ModelStats, ChartData, ChartDataSet
)
...
class StatsService:
    ...
    def get_overall_stats(self, config: StudyConfigData) -> OverallStats:
        ...
        # 平均答题时间
        avg_time = self.db.query(func.avg(Response.time_spent)).filter(
            Response.time_spent.isnot(None)
        ).scalar()
    ...
    def _get_question_stats(self, config: StudyConfigData) -> List[QuestionStats]:
        ...
        for question in config.questions:
            ...
            stats.append(QuestionStats(
                ...
                prompt=question.prompt[:100] + "..." if len(question.prompt) > 100 else question.prompt,
                total_responses=len(responses),
                picks_by_index=dict(picks_by_index),
                picks_by_model=dict(picks_by_model),
                average_time_spent=round(total_time / time_count, 2) if time_count > 0 else None
            ))
    ...
    def _get_model_stats(self, config: StudyConfigData) -> List[ModelStats]:
        ...
        model_stats = {model: {"total": 0, "emotion": 0, "content": 0} for model in all_models}
        ...
        for question in config.questions:
            ...
            for r in responses:
                if r.selected_index is not None and r.selected_index < len(question.models):
                    model_name = question.models[r.selected_index]
    ...
    def get_dashboard_stats(self) -> Dict:
        ...
        today_participants = self.db.query(Participant).filter(
        ...
        # 最近参与的用户（最近10个）
        recent_participants = self.db.query(Participant).order_by(
            Participant.started_at.desc()
        ).limit(10).all()
        ...
        for p in recent_participants:
            ...
            recent_list.append({
                "id": p.id[:8] + "...",
                "created_at": p.started_at.strftime("%m-%d %H:%M") if p.started_at else "",
                "progress": progress,
    ...
    def get_participant_consistency_analysis(self, config: StudyConfigData) -> Dict:
            ...
            for question in config.questions:
                ...
                if qid.endswith("-1"):  # 情感问题
                    ...
                    if content_id in response_map and qid in response_map:
                        total_groups += 1
                        if response_map[qid].selected_index == response_map[content_id].selected_index:
                            consistent_count += 1
        ...
        return {
            ...
            "average_consistency_rate": sum(d["consistency_rate"] for d in consistency_data) / len(consistency_data) if consistency_data else 0,
